...
function timeSpanToString(startDate, endDate) {
  ...
  const ms = diffDate.getUTCMilliseconds().toString().padStart(3, '0');
...
function angleBetweenClockHands(date) {
  ...
  // y радиан = x градусов * PI / 180;

  const minutes = date.getUTCMinutes();
  const hours = date.getUTCHours();

  let hAngle = (60 * (hours % 12) + minutes) / 2;
  hAngle = Math.min(hAngle, 360 - hAngle);
  let mAngle = 6 * minutes;
  mAngle = Math.min(mAngle, 360 - mAngle);

  const angle = (Math.abs(hAngle - mAngle) * Math.PI) / 180;
